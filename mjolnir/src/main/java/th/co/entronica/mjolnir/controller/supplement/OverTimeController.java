...
//        model.put("formhome", "home");
    ...
    @RequestMapping(value = "editForm2/{OTId}", method = RequestMethod.GET)
    public String userEdit2(ModelMap model, @PathVariable String OTId, @RequestParam(value="empId", required = true) String empId) {
        OverTime overtimeForm = (OverTime) overtimeManager.get(OTId);
        model.addAttribute("id", overtimeForm.getId());
        model.addAttribute("userInfo", userInfo);
    ...
    public String userUpdate2(@ModelAttribute("overtimeForm") OverTimeForm OTForm, ModelMap model, HttpServletResponse response) throws IOException {
        String empId = OTForm.getEmpId();
        overtimeManager.update(OTForm);
