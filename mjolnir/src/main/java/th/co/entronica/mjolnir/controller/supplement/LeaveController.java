...
//        model.put("formhome", "home");
    ...
    @RequestMapping(value = "editForm2/{LeaveID}", method = RequestMethod.GET)
    public String userEdit2(ModelMap model, @PathVariable String LeaveID, @RequestParam("empId") String empId, @RequestParam("start") String start) {
        Leave leaveForm = (Leave) leavemanager.get(LeaveID);
        model.addAttribute("id", leaveForm.getId());
        model.addAttribute("userInfo", userInfo);
        model.put("action", "update2");
        model.put("leaveForm", leaveForm);
        model.addAttribute("empId", empId);
        model.addAttribute("start", start);
        model.put("sw", start);
    ...
    @RequestMapping(value = "editForm2/update2", method = RequestMethod.POST)
    public String userUpdate2(@ModelAttribute("leaveForm") LeaveForm leaveForm, @ModelAttribute("sw") String start, ModelMap model, HttpServletResponse response) throws IOException {
        String empId = leaveForm.getEmpId();
        leavemanager.update(leaveForm);
        response.sendRedirect("../../../main/notice/personal");
        return null;
    }
