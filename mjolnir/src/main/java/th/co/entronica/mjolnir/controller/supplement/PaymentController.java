...
//        model.put("formhome", "home");
    ...
    @RequestMapping(value = "editForm2/{paymentId}", method = RequestMethod.GET)
    public String userEdit2(ModelMap model, @PathVariable String paymentId, @RequestParam(value="empId", required = true) String empId) {
        Payment paymentForm = (Payment) paymentManager.get(paymentId);
        model.addAttribute("id", paymentForm.getId());
        model.addAttribute("userInfo", userInfo);
    ...
    public String userUpdate2(@ModelAttribute("paymentForm") PaymentForm paymentForm, ModelMap model, HttpServletResponse response) throws IOException {
        String empId = paymentForm.getEmpId();
        paymentManager.update(paymentForm);
